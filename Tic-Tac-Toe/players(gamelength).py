...
class MinimaxPlayer(WinsBlocksForksPlayer):
    ...
    def minimax(self, board: str) -> Tuple[int, Tuple[int]]:
        """
        ...
        """
        """ Your Code Here """
        moves_available = validMoves(board)
        current_player = whoseMove(board)
        gamelength = {"length": 0}

        if(self.isTerminal(board)):
            if(theWinner(board) == 'X'):
                return(None, (10 + gamelength["length"], gamelength["length"]))

            elif(theWinner(board) == 'O'):
                return(None, (-10 + gamelength["length"], gamelength["length"]))

            else:
                return(None, (gamelength["length"], gamelength["length"]))
        ...
        if(current_player == 'X'):
            ...
            for move in moves_available:
                newBoard = setMove(board, move, current_player)
                if(value < self.minimax(newBoard)[1][0]):
                    bestmove = move
                    gamelength["length"] = gamelength["length"] + 1
                    value = self.minimax(newBoard)[1][0]

            return(bestmove,(value, gamelength["length"]))
